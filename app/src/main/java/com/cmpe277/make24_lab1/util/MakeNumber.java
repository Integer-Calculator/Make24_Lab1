package com.cmpe277.make24_lab1.util;

/**
 * Created by shiva on 3/11/18.
 */
...
public class MakeNumber {
     ...
     */
    public static String getUtilSolution(int a, int b, int c, int d) {
        int[] n = { a, b, c, d };
        char[] o = { '+', '-', '*', '/' };
        for (int w = 0; w < 4; w++) {
            for (int x = 0; x < 4; x++) {
    ...
    public static String getSolution(int a, int b, int c, int d) {
        ...
        String result = getUtilSolution(a, b, c, d);
        if (null != result) {
            //System.out.println("Binggo! " + result);
            return result;
        } else {
            return "Sorry, there are actually no solutions";
